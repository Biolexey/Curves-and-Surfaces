...
def N(r, knot, i, u):                  # ノット重複を許容するBasis関数
  if r > 0:
    if (i<0 or knot[i] == knot[i+r]) and (i+r+1>len(knot)-1 or knot[i+1] == knot[i+r+1]):
      return 0

    elif (i<0 or knot[i] == knot[i+r]):
      return (knot[i+r+1]-u)*N(r-1,knot,i+1,u)/(knot[i+r+1]-knot[i+1])

    elif (i+r+1>len(knot)-1 or knot[i+1] == knot[i+r+1]):
...
def main():                              # main関数
  ...
  for i in range(len(knot)-n-1):         # 反復
    Basis(canvas, n, knot, i).BasisdrawCurve(knot[0], knot[-1], offset, nop = 2000) # basis多項式の描画

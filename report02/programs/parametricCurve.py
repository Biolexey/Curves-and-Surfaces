...
class ParametricCurve(object):           # ParametricCurveクラスの定義
  ...
  def drawCurve(self, ts = 0, te = 0, segment=-1, offset = np.array((0, 0)), nop = 128):
    ...
    prevPnt = self.evaluate(ts, segment) + offset # 最初の点の座標 (ワールド座標系)
    prevIn  = self.canvas.inside(prevPnt) # 最初の点が表示領域内にあるか否か
    for i in range(nop):                 # 線分列の数だけ反復
      currPnt = self.evaluate(ts + dt*(i+1), segment) + offset # 次の点の座標 (ワールド座標系)
  ...
  def BasisdrawCurve(self, ts = 0, te = 0, offset = np.array((0, 0)), nop = 128):# Basis関数描画メソッド
                                         # ParametricCurveオブジェクトの描画メソッド
    '''
    ts, te - 描画対象のパラメタ値 (開始,終了)，省略時 0, 0
    offset - 平行移動ベクトル，省略時 (0, 0)
